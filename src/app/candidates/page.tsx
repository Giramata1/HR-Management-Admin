...
import { Search, Bell, ChevronDown, Plus, Pencil, Trash2, X, CheckCircle, XCircle, Info } from 'lucide-react';
...
const CandidateList = () => {
  ...
  if ((loading && candidateList.length === 0) || !loggedInUser) {
    return (
        ...
        <div className="flex items-center space-x-3 sm:space-x-4">
          ...
          {loggedInUser && (
            <div className="flex items-center space-x-2 sm:space-x-3 border border-gray-200 dark:border-gray-700 px-2 sm:px-3 py-2 rounded-md bg-white dark:bg-gray-800">
                <div className="rounded-full w-8 h-8 sm:w-10 sm:h-10 bg-indigo-600 flex items-center justify-center text-white font-bold text-sm">
                ...
                paginatedCandidates.map((c) => (
                  <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 sm:px-6 py-3 flex items-center space-x-3">
                      <div className="rounded-full w-8 h-8 sm:w-9 sm:h-9 bg-purple-600 flex items-center justify-center text-white font-semibold">
                        {getInitials(c.name)}
                      </div>
                      <span>{c.name}</span>
                    </td>
