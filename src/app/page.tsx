...
function RegisterForm({ onBack }: { onBack: (e: React.MouseEvent<HTMLButtonElement>) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('userName', fullName || form.fullName.trim()); // Use response or form data
      localStorage.setItem('userEmail', email || form.email);
      localStorage.setItem('userRole', role || form.role);
      
      console.log('Stored user data after registration:', { // Debug log
        token,
        fullName: fullName || form.fullName.trim(),
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="relative">
            <select
              name="role"
              required
              className="w-full px-4 pt-6 pb-2 text-black dark:text-white bg-white dark:bg-gray-800 border border-purple-500 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 appearance-none"
              value={form.role}
              onChange={handleChange}
            >
...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      localStorage.setItem('token', token);
      localStorage.setItem('userName', fullName || userEmail); // Use fullName from response
